...
static void assign()
{
    ...
    if (token < 'a' || token > 'f') {
        ERROR("Expected variable on the left-hand side of assignment\n");
        exit(EXIT_FAILURE);
    }
    char var = token;
    next_token(); 
...
static void read()
{
    ...
    next_token();
    if (token < 'a' || token > 'f') {
        ERROR("Expected variable after '?' in read statement\n");
        exit(EXIT_FAILURE);
    }
    char var = token;
    next_token();
...
static void print()
{
    ...
    next_token();
    if (token < 'a' || token > 'f') {
        ERROR("Expected variable after '%' in print statement\n");
        exit(EXIT_FAILURE);
    }
    char var = token;
    next_token();
...
static void program()
{
	stmtlist();
	if (token != '!') {
		ERROR("Program error.  Expected '!' at the end of the program but found '%c'\n", token);
		exit(EXIT_FAILURE);
	};
}
